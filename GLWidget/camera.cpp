...
void CCamera::MoveForwards( GLfloat Distance )
{
	...
	if (CheckCollision(Vec3D(Position.x, Position.y, Position.z), Vec3D(MoveVector.x, MoveVector.y, MoveVector.z)))
...
void CCamera::StrafeRight ( GLfloat Distance )
{
	...
	if (CheckCollision(Vec3D(Position.x, Position.y, Position.z), Vec3D(MoveVector.z, MoveVector.y, MoveVector.x)))
		return;
